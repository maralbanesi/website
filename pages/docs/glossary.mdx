...
An algorithm or set of rules that Mina nodes all agree upon when deciding to update the state of the network. Rules may include what data a new block can contain, and how nodes are selected and rewarded for adding a block. Mina implements Ouroboros Proof-of-Stake as its consensus mechanism.
...
The type of consensus algorithm Mina implements to allow nodes to agree upon the state of the network. Proof-of-Stake (PoS) allows nodes to [stake](#staking) mina on the network to increase their chance of being selected as the next block producer.
